...
public sealed class RelativeStrengthIndexTests
{
    ...
    public void CalculateRelativeStrengthIndex_ReturnsSeriesWithinBounds()
    {
        ...
        var lookback = TaLibCore.GetRelativeStrengthIndexLookback(period);
        Assert.Equal(values.Length, rsi.Length);
        Assert.All(rsi.Take(lookback), value => Assert.True(double.IsNaN(value)));
        Assert.All(rsi.Skip(lookback), value => Assert.InRange(value, 0.0, 100.0));
    }
