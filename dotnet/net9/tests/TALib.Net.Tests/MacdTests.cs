...
public sealed class MacdTests
{
    ...
    public void CalculateMacd_ReturnsAlignedSeries()
    {
        ...
        var expectedLength = values.Length - TaLibCore.GetMacdLookback(fast, slow, signal);

        Assert.Equal(expectedLength, macd.Macd.Length);
        Assert.Equal(expectedLength, macd.Signal.Length);
        Assert.Equal(expectedLength, macd.Histogram.Length);
